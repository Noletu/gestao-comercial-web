...
- **Desde a issue #3 (banco de teste dedicado): `npm test -w api` roda contra
  `gestao_comercial_test`, um database SEPARADO no mesmo cluster Postgres — não
  toca mais no estoque real do Lucas.** Antes da primeira rodada (ou depois de
  clonar o repo), rode `npm run db:test:setup` (dentro de `apps/api`): cria
  `apps/api/.env.test` (não versionado, a partir de `.env.test.example`), cria
  o database de teste, aplica schema + RLS e provisiona o role `app_user`.
  Idempotente, pode rodar de novo a qualquer momento. `apps/api/vitest.setup.ts`
  carrega **só** `.env.test` e trava a suíte (`process.exit(1)`, sem rodar um
  único teste) se o arquivo não existir ou se `DATABASE_URL`/
  `DIRECT_DATABASE_URL` apontarem para um database que não termine em `_test`
  — ver `specs/banco-de-teste-dedicado.md`.
  `npm run db:seed` e `npm run db:reset` continuam agindo sobre o banco de
  **desenvolvimento** (fora de escopo da issue #3, de propósito — são comandos
  disparados de propósito, não a suíte que roda a toda hora). Rodá-los ainda
  apaga o estoque real e exige recuperar com
  `npm run db:import-inventory -- "<caminho do .xlsx do Lucas>"` (de dentro de
  `apps/api`) antes de devolver acesso.
